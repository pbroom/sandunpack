...
- The next recommended step is to prepare the upstream repro/report now using `z-tests-manual/test-results-6.md` as the current version matrix. Only probe `7.3.2` or `7.3.3` later if upstream explicitly wants to know where the earlier module-resolution failure stops. Keep `minimal-startup-race-client` as the small control fixture, `timeout-restart-repro` as the timeout control, and `color-kit-plane-api-repro` as the heavy validation fixture.

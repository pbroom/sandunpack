...
function summarizeSetup(setup: SandboxSetup): Record<string, unknown> {
  const filePaths = Object.keys(setup.files);
...
export class SandpackRuntime extends SandpackClient {
  ...
  constructor(
    ...
    emitDebugEvent("client:runtime:init", {
      bundlerURL: this.bundlerURL,
      startRoute: options.startRoute,
      ...summarizeSetup(sandboxSetup),
    });
    ...
    this.unsubscribeGlobalListener = this.iframeProtocol.globalListen(
      (mes: SandpackMessage) => {
        ...
        emitDebugEvent("client:runtime:initialized", {
          bundlerURL: this.bundlerURL,
          ...summarizeSetup(this.sandboxSetup),
        });
  ...
  updateSandbox(
    ...
    emitDebugEvent("client:runtime:update-sandbox", {
      isInitializationCompile: Boolean(isInitializationCompile),
      status: this.status,
      ...summarizeSetup(this.sandboxSetup),
    });
